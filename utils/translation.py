def get_translations(lang):
    translations = {
        "fr": {
            ...
            "agent_tool_approval": "Contrôle humain des appels d'outils",
            "agent_tool_approval_hint": "Avant chaque exécution d'outil, affiche une boîte de dialogue pour approuver, modifier ou annuler.",
            "tool_approval_title": "Approuver l'appel d'outil",
            "tool_approval_about_to_call": "L'agent s'apprête à exécuter l'outil suivant :",
            "tool_approval_args_label": "Paramètres (modifiables) :",
            "tool_approval_feedback_label": "Votre message à l'agent (optionnel) :",
            "tool_approval_feedback_placeholder": "Décrivez une alternative, une correction ou une alerte...",
            "tool_approval_btn_approve": "Approuver",
            "tool_approval_btn_approve_all": "Approuver tout",
            "tool_approval_btn_feedback": "Envoyer un feedback",
            "tool_approval_btn_cancel_loop": "Annuler le traitement",
            "tool_approval_cancelled": "Traitement annulé par l'utilisateur.",
            ...
            "process_vartype_color":   "Couleur",
            "process_vartype_number":  "Nombre",
            "process_vartype_boolean": "Booléen",
            ...
            "process_step_deleted_title": "Étape supprimée",
            "process_step_deleted_vars": "{n} variable(s) supprimée(s) car elles n'étaient utilisées que par cette étape.",
        ...
        "en": {
            ...
            "agent_tool_approval": "Human control of tool calls",
            "agent_tool_approval_hint": "Before each tool execution, show a dialog to approve, modify, or cancel.",
            "tool_approval_title": "Approve tool call",
            "tool_approval_about_to_call": "The agent is about to run the following tool:",
            "tool_approval_args_label": "Parameters (editable):",
            "tool_approval_feedback_label": "Your message to the agent (optional):",
            "tool_approval_feedback_placeholder": "Describe an alternative, a correction, or a warning...",
            "tool_approval_btn_approve": "Approve",
            "tool_approval_btn_approve_all": "Approve all",
            "tool_approval_btn_feedback": "Send feedback",
            "tool_approval_btn_cancel_loop": "Cancel task",
            "tool_approval_cancelled": "Task cancelled by user.",
            ...
            # Agent prompts
            "agent_system_prompt": (
                "You are an expert QGIS agent embedded in a running QGIS session. "
                "Use the available tools to fulfill the user's request.\n\n"

                "## Execution\n"
                "- Work step by step; analyze each tool result before the next call.\n"
                "- On failure (success: false), explain why and suggest an alternative.\n"
                "- Before referencing fields, call get_layer_fields first.\n"
                "- Before any metric operation (buffer, distance…), check CRS with get_layer_info. "
                "If is_geographic=true, reproject to a metric CRS first — "
                "meter distances on a degree CRS produce silently wrong results.\n"
                "- Use run_pyqgis_code only when no native tool covers the need.\n\n"

                "## run_pyqgis_code\n"
                "- Use print() to verify results (captured in print_output).\n"
                "- Never print() inside a loop — accumulate, then print once after:\n"
                "  ✗ for f in layer.getFeatures(): print(f['name'])\n"
                "  ✓ names=[f['name'] for f in layer.getFeatures()]; print(f'{len(names)}: {names[:5]}')\n"
                "- Never call iface.messageBar().\n\n"

                "## Language\n"
                "Reply in the user's language — detect it from their message only, "
                "not from layer names, field names or tool results.\n\n"

                "## Final reply\n"
                "Summarize what was done. Explicitly list any non-obvious parameter values applied, "
                "especially defaults chosen because the user did not specify them "
                "(classification mode, color ramp, buffer distance, dissolve, cap style…) — "
                "this lets the user re-run with adjustments if needed. "
                "Act first, then report — never ask for confirmation beforehand."
            ),
            "agent_system_prompt_canvas_rules": (
                "After any style, symbology, label or visual change, call capture_map_canvas to verify the result.\n"
            ...
            "process_vartype_color":   "Color",
            "process_vartype_number":  "Number",
            "process_vartype_boolean": "Boolean",
            ...
            "process_step_deleted_title": "Step deleted",
            "process_step_deleted_vars": "{n} variable(s) removed — they were only used by this step.",
